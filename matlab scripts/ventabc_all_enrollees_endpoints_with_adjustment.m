...
predictor_4 = true_false_nan(raw_table.was_sbt_alerted(is_control_or_int) ) .* 1.0;
...
ordinal_response = ordinal(ve_outcome(is_control_or_int),{'1','2','3' },[],[0,1,2,3]);
nominal_response = categorical( ve_outcome(is_control_or_int) );

[B,dev,stats] = mnrfit([ predictor_1, predictor_2, predictor_3, predictor_4 ], nominal_response );
stats.p(2, :)
    % the first column is odds died/extubated
    % the second column is odds trached/extubated
    % coeff -0.2863   -0.2620
    % p values = 0.2122    0.4331
...
for i = 1:length(outcomes)
    ...
    nominal_response = categorical(outcomes(i).classifications(is_control_or_int));
%     ordinal_response = ordinal(outcomes(i).classifications(is_control_or_int),{'1','2','3','4'},[],[9,19,29,39,48]);
    ...
    % now calculate the t-test between proportions
