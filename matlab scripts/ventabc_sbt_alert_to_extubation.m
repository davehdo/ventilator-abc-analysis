...
param = "time from SBT to leaving ICU";
...
time_interval = cell_of_strings_to_numerical_array(raw_table.t_sbt_to_transfer_out_of_unit_or_last_hosp_date);
...
n_where_t_lte_zero = sum([ values_int;  values_control] <= 0);
if n_where_t_lte_zero > 0
    disp(['  Warning: there are ' num2str(n_where_t_lte_zero ) ' dubious values where t interval <= 0']);
