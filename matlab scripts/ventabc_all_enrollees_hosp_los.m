...
is_censored = ismember(raw_table.visit_outcome_was_expired, 'true');

time_interval = cell_of_strings_to_numerical_array( raw_table.LOS_NUM );
...
disp('Cox Adjusted by APACHE IV and Pressor dep:');
...
disp('Cox Unadjusted, not censoring death');
...
disp(['  unadj, no censoring death  m1 = ' num2str(nanmean( log(values_control))) '  m2 = ' num2str(nanmean( log(values_int)))  '  p = ' num2str(ttest2_log_p)] )

[h, ttest3_log_p, ci, ttest3_log_stats] = ttest2( log(censored_values_int ), log(censored_values_control), 'Tail', 'left');
disp(['  unadj, censored death  m1 = ' num2str(nanmean( log(censored_values_control))) '  m2 = ' num2str(nanmean( log(censored_values_int)))  ' p = ' num2str(ttest3_log_p)] )

[h, ttest4_log_p, ci, ttest4_log_stats] = ttest2( log(cap(values_int, 60) ), log(cap(values_control, 60)), 'Tail', 'left');
disp(['  unadj, no censoring death, cap 60  m1 = ' num2str(mean_cap( log(values_control), log(60) )) '  m2 = ' num2str(mean_cap( log(values_int), log(60)))  '  p = ' num2str(ttest4_log_p)] )

[h, ttest5_log_p, ci, ttest5_log_stats] = ttest2( log(cap(censored_values_int, 60 ) ), log(cap(censored_values_control, 60)), 'Tail', 'left');
disp(['  unadj, censored death, cap 60  m1 = ' num2str(mean_cap( log(censored_values_control), log(60) )) '  m2 = ' num2str(mean_cap( log(censored_values_int), log(60)))  ' p = ' num2str(ttest5_log_p)] )
